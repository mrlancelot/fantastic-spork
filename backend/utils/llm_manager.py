...
class LLMProfile(Enum):
    ...
    FAST = "fast"           # Fast, cost-effective for simple tasks
...
class LLMManager:
    ...
    # Default configurations for each profile - all using Gemini 2.5 Flash to avoid rate limits
    _profile_configs = {
        LLMProfile.FAST: {
    ...
    def _create_llm_instance(self, config: Dict[str, Any]) -> Union[GoogleGenAI, OpenAI, CerebrasLLM]:
        ...
        try:
            ...
            # Try fallback to Google Gemini Fast if not already using it
            if provider != LLMProvider.GOOGLE_GEMINI:
