        ...
        try:
            ...
            if "state" in ctx_state:
                ...
                final_progress = {
                    ...
                    "step": "activities",
                    "details": {
                        "flights_found": len(flights_data.get("flights", [])),
                        "hotels_found": len(hotels_data.get("hotels", [])),
                        "restaurants_found": len(restaurants_data.get("restaurants", [])),
                        "activities_planned": activities_count,
                        "price_ranges": {}
                    }
                }

                if flights_data.get("best_price"):
                    final_progress["details"]["price_ranges"]["flights"] = {
                ...
                if hotels_data.get("best_price"):
                    final_progress["details"]["price_ranges"]["hotels"] = {
            ...
            logger.warning(f"Could not extract progress from context: {e}")
        ...
        for day in days:
            for activity in day.get("activities", []):
                activity_data = {
                    ...
                    "duration": parse_duration_to_hours(activity.get("duration")),
...
async def get_itinerary_status(job_id: str):
    ...
    try:
        ...
        if isinstance(result_data, dict) and "progress" in result_data:
            response["progress"] = result_data["progress"]
...
async def create_itinerary_sync(request: ItineraryRequest):
    ...
    try:
        ...
        logger.info(f"Content: {str(result)[:1000]}...")
        logger.info("-"*60)
            ...
            if itinerary_data:
                ...
                while f"day_{day_num}" in itinerary_data:
                    ...
                    for idx, activity in enumerate(day_data.get("activities", []), 1):
                        activities.append({
                            "time": f"{9 + idx * 2:02d}:00",
                            "title": activity[:50] if len(activity) > 50 else activity,
