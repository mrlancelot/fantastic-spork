...
from core.google_flights_scraper import GoogleFlightsRoundTripScraper
...
class FlightService:
    ...
    def __init__(self):
        self.logger = setup_logger('FlightService')
    ...
    async def search(self, request: Dict) -> Dict:
        ...
        """
        self.logger.info(f"Processing flight search: {request['origin']} → {request['destination']}")
        
        try:
            # Use the scraper's search method
            ...
            self.logger.error(f"Flight search error: {e}", exc_info=True)
            return {
                'status': 'error',
                'error': str(e),
                'flights': [],
    ...
    def _is_reasonable_time(self, time_str: str) -> bool:
